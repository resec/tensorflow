...
usage() {
  ...
  echo "-a [android_abi] supported are ARMV5, ARMV7, ARM64, MIPS, MIPS64, X86, X86_64 [default=ARMV7]"
  exit 1
}
...
tensorflow/contrib/makefile/compile_android_protobuf.sh -a ${arch} -c
else
...
if [[ -z "${BUILD_TARGET}" ]]; then
    make -j"${JOB_COUNT}" -f tensorflow/contrib/makefile/Makefile \
         TARGET=ANDROID ANDROID_ARCH="${ARCH}" NDK_ROOT="${NDK_ROOT}" \
CC_PREFIX="${CC_PREFIX}" \
